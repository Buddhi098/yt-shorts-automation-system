...
from moviepy import ImageClip, CompositeVideoClip # Ensure these are imported
from moviepy.video.fx import Resize # If you use the effects version
from moviepy import ColorClip, CompositeVideoClip, concatenate_videoclips
from moviepy.video.fx import FadeIn, FadeOut # New in 2.x architecture

class VideoProcessor:
    def __init__(self, logger):
        self.config = settings.video
        self.logger = logger
        self.color_sets = [("yellow", "white"), ("lime", "white"), ("orange", "white")]
        self.hook_phrases = [
    ...
    def trim_random_clip(self, clip: VideoFileClip) -> VideoFileClip:
        safe_duration = clip.duration - self.config.safe_start_margin - self.config.safe_end_margin
    ...
    def merge_videos_with_text(self, clips: list[VideoFileClip], sentences: list[str], color_set: tuple) -> CompositeVideoClip:
        processed_clips = []
        ...
        for clip, sentence in zip(clips, sentences):
            ...
            dark_overlay = (ColorClip(clip.size, color=(0, 0, 0))
    ...
    def generate_hook_clip(self, color_set: tuple) -> CompositeVideoClip:
        ...
        txt_clip = TextClip(
            ...
            color=color_set[0]
        ).with_position("center").with_duration(self.config.video_duration)

        return CompositeVideoClip([txt_clip], size=(self.config.target_width, self.config.target_height), bg_color=(0, 0, 0))
    ...
    def select_random_videos(self, folder_path: str, count: int) -> list[VideoFileClip]:
        ...
        if len(video_files) < count:
            self.logger.warning(f"Only {len(video_files)} videos available, requested {count}. Adjusting count.")
        ...
        for file in selected_files:
            try:
                ...
                if clip.duration > 0 and clip.w > 0 and clip.h > 0:
                    clips.append(clip)
                    self.logger.info(f"Loaded video: {file.name} ({clip.duration:.1f}s)")
                else:
                    clip.close()
    ...
    def get_random_music(self, folder_path: str) -> AudioFileClip | None:
        ...
        try:
            ...
            self.logger.info(f"Loaded audio: {selected_file.name} ({audio_clip.duration:.1f}s)")
            return audio_clip
        except Exception as e:
    ...
    def add_music_to_video(self, video: VideoFileClip, audio: AudioFileClip) -> VideoFileClip:
        if audio.duration < video.duration:
            # Loop the audio to exceed or meet the video duration
    ...
    def add_logo_to_video(self, video: VideoFileClip, logo_path: str) -> VideoFileClip:
        ...
        logo_clip = (ImageClip(str(logo_file))
                    .with_fps(video.fps)
                    .with_duration(video.duration)
                    .resized(height=int(video.h * 0.1))
                    # Position: 'center' horizontally, 
                    # and (video height - logo height - padding) vertically
                    .with_position(('center', video.h - int(video.h * 0.1) - bottom_padding)))
